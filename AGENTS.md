...
- Paper state and logs are gitignored runtime files: `paper_wallet.json`, `fileBalance.txt`, `fileCicloStart.txt`, `cronoMacd.txt`. `fileCicloStart.txt` stores the buy price that gates trailing sells (`write_ciclostart`).
