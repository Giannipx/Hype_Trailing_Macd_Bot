...
- **Paper trading** di default: prezzi reali da mainnet, wallet simulato in `paper_wallet.json`. Nessuna chiave richiesta.
...
Il bot gira in un loop infinito (termina con Ctrl+C). Stato e cronologia vengono scritti nei file runtime `paper_wallet.json`, `fileBalance.txt`, `fileCicloStart.txt`, `cronoMacd.txt` (gitignored).
